...
import {
  ...
  uploadPracticeMedia,
  uploadUserDocument,
  uploadPracticeComplianceDocument,
...
const handleMulterError = (err, req, res, next) => {
  if (err) {
    ...
    if (err.code === 'LIMIT_FILE_SIZE') {
      return res
        .status(400)
        .json({ message: 'File too large. Please check file size limits.' });
    }
    if (err.code === 'LIMIT_UNEXPECTED_FILE') {
      ...
      if (err.field === 'file') {
        return res.status(400).json({
          message:
            'Field name mismatch. For multiple files, use field name "file" and send multiple files with the same field name.',
...
router.post(
  '/user/media',
  authMiddleware,
  handleUpload(upload.single('file'), uploadUserMedia)
);
router.post(
  '/practice/media',
  authMiddleware,
  handleUpload(upload.array('file', 5), uploadPracticeMedia)
);
router.post(
  '/user/document',
  authMiddleware,
  handleUpload(upload.array('file', 5), uploadUserDocument)
);
...
router.post(
  '/practice/compliance',
  authMiddleware,
  handleUpload(upload.single('file'), uploadPracticeComplianceDocument)
);
