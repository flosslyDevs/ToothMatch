...
export async function scheduleInterview(req, res) {
  ...
  try {
    ...
    if (!timeRegex.test(time)) {
      return res.status(400).json({
        message:
          'Invalid time format. Use 24-hour format (HH:MM), e.g., "14:30"',
      });
    }
...
export async function requestReschedule(req, res) {
  ...
  try {
    ...
    if (!candidate || candidate.role !== 'candidate') {
      return res
        .status(403)
        .json({ message: 'Only candidates can request reschedule' });
    }
    ...
    if (!requestedDate || !requestedTime) {
      return res.status(400).json({
        message:
          'Missing required fields: requestedDate and requestedTime are required',
      });
    }
...
export async function approveReschedule(req, res) {
  ...
  try {
    ...
    if (!practice || practice.role !== 'practice') {
      return res
        .status(403)
        .json({ message: 'Only practices can approve reschedule requests' });
    }
...
export async function declineInterview(req, res) {
  ...
  try {
    ...
    if (!candidate || candidate.role !== 'candidate') {
      return res
        .status(403)
        .json({ message: 'Only candidates can decline interviews' });
    }
    ...
    if (interview.candidateUserId !== candidateUserId) {
      return res
        .status(403)
        .json({ message: 'You can only decline your own interviews' });
    }
...
export async function acceptInterview(req, res) {
  ...
  try {
    ...
    if (!candidate || candidate.role !== 'candidate') {
      return res
        .status(403)
        .json({ message: 'Only candidates can accept interviews' });
    }
    ...
    if (interview.candidateUserId !== candidateUserId) {
      return res
        .status(403)
        .json({ message: 'You can only accept your own interviews' });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: 'This interview cannot be accepted' });
    }
