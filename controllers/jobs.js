  ...
  try {
    ...
    let filteredLocumShifts = allLocumShifts.filter((shift) => {
      ...
      if (payRangeMin || payRangeMax) {
        ...
        passesPayFilter =
          (hourlyRate >= minPay && hourlyRate <= maxPay) ||
      ...
      if (salaryPreferenceMin || salaryPreferenceMax) {
        ...
        const passesSalaryFilter =
          (hourlyRate >= minSalary && hourlyRate <= maxSalary) ||
      ...
      if (workingPattern) {
        ...
        if (shiftLabel) {
          if (desiredPattern === 'day-shift' && shiftLabel !== 'day-shift')
            return false;
          if (desiredPattern === 'night-shift' && shiftLabel !== 'night-shift')
            return false;
        } else {
          const timeLower = (shift.time || '').toLowerCase();
          if (desiredPattern === 'day-shift') {
            if (
              !timeLower.includes('day') &&
    ...
    let filteredPermanentJobs = allPermanentJobs.filter((job) => {
      ...
      if (workingPattern) {
        ...
        if (shiftLabel) {
          ...
          if (desiredPattern === 'day-shift') {
            if (
              !workingHoursLower.includes('day') &&
              !workingHoursLower.includes('morning') &&
              !workingHoursLower.includes('afternoon')
            ) {
              return false;
            }
          } else if (desiredPattern === 'night-shift') {
            if (
              !workingHoursLower.includes('night') &&
              !workingHoursLower.includes('evening')
            ) {
              return false;
            }
          }
        ...
        filters: {
          ...
          payRange:
            payRangeMin || payRangeMax
              ? { min: payRangeMin, max: payRangeMax }
              : null,
          searchRadius: searchRadius || null,
          salaryPreference:
            salaryPreferenceMin || salaryPreferenceMax
              ? { min: salaryPreferenceMin, max: salaryPreferenceMax }
              : null,
        },
      },
    });
  ...
  try {
    ...
    let filtered = candidates.filter((c) => {
      ...
      if (jobType) {
        if (
          !pref.jobType ||
          !pref.jobType.toLowerCase().includes(String(jobType).toLowerCase())
        )
          return false;
      }
      ...
      if (workingPattern) {
        if (
          ...
          !pref.workingPattern
            .toLowerCase()
            .includes(String(workingPattern).toLowerCase())
        )
          return false;
      }
        ...
        filters: {
          ...
          salaryPreference:
            salaryPreferenceMin || salaryPreferenceMax
              ...
              : null,
          searchRadius:
            practiceLat && practiceLng && searchRadius
              ? { miles: searchRadius, practiceLat, practiceLng }
              : null,
        },
      },
    });
