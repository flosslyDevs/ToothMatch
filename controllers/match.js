...
function parseSalaryRange(s) {
    ...
    const vals = nums.map(n => Number(n.replace(/,/g, ''))).filter(v => !isNaN(v));
...
function scoreCandidateToJob(candidate, job, jobType) {
    ...
    const jtMatch = (pref.jobType && job.jobType && normalizeStr(pref.jobType) === normalizeStr(job.jobType)) ? 1 : 0;
    score += jtMatch * 10;
    const wp = normalizeStr(pref.workingPattern);
    const wh = normalizeStr(job.workingHours || job.time);
    if (wp && wh && (wh.includes(wp.split('-')[0]) || wh.includes(wp))) score += 10;

    // pay/salary match (40)
    let candMin = Number(pref.payMin) || 0;
    let candMax = Number(pref.payMax) || Number(pref.hourlyRate) || 0;
    let jobMin = 0; let jobMax = 0;
    if (jobType === 'permanent') {
        const sr = parseSalaryRange(job.salaryRange);
        jobMin = sr.min || 0; jobMax = sr.max || 0;
    } else {
        jobMin = Number(job.hourlyRate) || Number(job.dayRate) || 0;
        jobMax = jobMin;
    }
    if ((jobMax && candMin && jobMax >= candMin) || (jobMin && candMax && jobMin <= candMax) || (!candMin && !candMax)) score += 40;
    ...
    if (job.PracticeProfile && job.PracticeProfile.PracticeLocations && job.PracticeProfile.PracticeLocations.length > 0) score += 10;
...
async function ensureMatchIfMutual(actorUserId, targetType, targetId) {
    ...
    if (targetType === 'candidate') {
        ...
        const job = await model.findByPk(targetId, { include: [{ model: PracticeProfile, include: [{ model: PracticeLocation, required: false }] }] });
        if (!job) return null;
        const candidateLike = await MatchLike.findOne({ where: { targetType, targetId, decision: 'like' } });
        ...
        const score = scoreCandidateToJob(candidate, job, targetType === 'locum' ? 'locum' : 'permanent');

        const existing = await Match.findOne({ where: { candidateUserId, practiceUserId, targetType, targetId } });
        if (existing) return existing;
        return await Match.create({ candidateUserId, practiceUserId, targetType: targetType === 'locum' ? 'locum' : 'permanent', targetId, score });
    }
}
...
export async function likeTarget(req, res) {
    ...
    if (!['locum','permanent','candidate'].includes(targetType)) return res.status(400).json({ message: 'invalid targetType' });
    if (!['like','pass'].includes(decision)) return res.status(400).json({ message: 'invalid decision' });
    try {
        const like = await MatchLike.create({ actorUserId, targetType, targetId, decision });
        let match = null;
        if (decision === 'like') {
...
export async function getMatches(req, res) {
    ...
    try {
        const where = { [Op.or]: [{ candidateUserId: userId }, { practiceUserId: userId }], status: 'matched' };
        const { count, rows } = await Match.findAndCountAll({ where, order: [['createdAt','DESC']], limit: parseInt(limit), offset: parseInt(offset) });
        ...
        return res.status(200).json({
            ...
            data: {
                total: count,
                matches: enriched,
                pagination: { page: parseInt(page), limit: parseInt(limit), totalPages: Math.ceil(count / parseInt(limit)) }
            }
        });
    } catch (error) {
